...
import com.hakz.www.redbook.activity.BookInfoAddActivity;
...
public class BookRecyclerAdapter extends RecyclerView.Adapter<BookRecyclerViewHolder> {
    ...
    public BookRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new BookRecyclerViewHolder(inflater.inflate(R.layout.activity_search_book_item,
                parent, false));
    }
    ...
    public void onBindViewHolder(BookRecyclerViewHolder holder, final int position) {
        ...
        holder.cardView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                List<Book> books = DataSupport.where("isbn13 = ?", list.get(position).getIsbn13()
                ).find(Book.class);
                // 如果图书已添加
                if (books.size() > 0) {
